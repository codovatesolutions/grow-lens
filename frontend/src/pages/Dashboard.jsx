...
const Widget = ({ title, icon: Icon, action, className = "", tid, children }) => (
...
// ---------- Score gauge (0-100) ----------
const ScoreGauge = ({ value = 0, label, color = "primary" }) => {
...
// ---------- Growth Trend Widget ----------
function GrowthTrendWidget({ trend }) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl md:text-4xl font-bold tracking-tight mt-1">
            {greeting}, {user?.name?.split(" ")[0] || "there"} <span className="text-primary">·</span>
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {totals?.complete_scans ?? 0} scans · {totals?.leads ?? 0} leads · {tasks_summary?.open ?? 0} open tasks
          </p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <Widget title="Recent Scans" icon={History} tid="widget-recent-scans"
          action={<Link to="/reports" className="text-xs text-primary hover:underline">View all</Link>}
        ...
        <Widget title="Competitor Alerts" icon={AlertTriangle} tid="widget-competitor-alerts" className="col-span-12 lg:col-span-4">
          {competitor_alerts.length === 0 ? (
            ...
            <div className="space-y-3">
              {competitor_alerts.map((a) => (
                <Link key={a.scan_id} to={`/scan/${a.scan_id}`} className="block p-2 rounded-md hover:bg-muted/40 transition">
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    You: {a.my_score} · Them: {a.their_score}
                  </div>
                </Link>
              ))}
            </div>
        ...
        <Widget title="AI Recommendations" icon={Lightbulb} tid="widget-ai-recs" className="col-span-12 lg:col-span-4">
          {ai_recommendations.length === 0 ? (
            ...
            <div className="space-y-3">
              {ai_recommendations.slice(0, 4).map((r, i) => (
                <Link key={i} to={`/scan/${r.scan_id}`} className="block group">
        ...
        <Widget title="Scan History" icon={History} tid="widget-scan-history"
          action={<Link to="/reports" className="text-xs text-primary hover:underline">Open reports</Link>}
                  ...
                  {recent_scans.map((s) => (
                    <tr key={s.id} className="hover:bg-muted/30 transition" data-testid={`history-row-${s.id}`}>
                      ...
                      <td className="py-2 px-2 text-right">
                        <Link to={`/scan/${s.id}`} className="text-primary hover:underline text-xs">Open →</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
